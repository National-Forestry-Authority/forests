...
async function updateMapLayers(instance, fireAlertsUrl, deforestationAlertsUrl, geometry) {
  ...
  const gfwListBoilerPlate = `
    <div id="gfw-list">
      <div class="sort-buttons">
        <button class="sort" data-sort="timeStamp">
          Sort by Timestamp
        </button>
        <button class="sort" data-sort="alertType">
          Sort by Alert Type
        </button>
      </div>
      <table>
  ...
  const gfwForm = document.querySelector('form');
  gfwForm?.parentElement?.appendChild(gfwListElement);
...
async function farmNfaPlotGfwApiMap(instance, mapType, gfwApiUrl, dateRange, geometry) {
  return new Promise(async (resolve, reject) => {
    ...
    const baseQuery = `SELECT latitude, longitude, ${dateParameter} FROM results`;
